...
sudo -E env PATH="$PATH" uv run ptf_nn/ptf_nn_agent.py \
...
sudo -E env PATH="$PATH" uv run ptf_nn/ptf_nn_test_bridge.py -ifrom veth1 -ito veth2 \
    &

sleep 5

sudo -E env PATH="$PATH" uv run ptf --test-dir ptf_nn/ptf_nn_test \
