...
def test_simple_tcp_packets_use_same_default_payload():
    tcp_payload = bytes(simple_tcp_packet()[Raw])
    tcpv6_payload = bytes(simple_tcpv6_packet()[Raw])
...
def test_payload_is_padded_to_packet_length():
    ...
    assert len(pkt) == 60
    assert bytes(pkt[Raw]) == b"abc" + bytes(range(3))
